    ...
    <div className="space-y-6">
      {showImportSuccess && (
        <div className="bg-primary/10 border border-primary/30 rounded-lg p-4 flex items-center space-x-3 backdrop-blur-sm">
          ...
          <button onClick={() => setShowImportSuccess(false)} className="text-primary hover:text-primary/80">×</button>
        </div>
      )}
          ...
          <table className="w-full">
            <thead className="bg-muted/30">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Reorder Level</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">ABC Class</th>
            ...
            <tbody className="divide-y divide-black/10 dark:divide-white/10">
              {Array.isArray(products) && products.map((product) => {
                ...
                return (
                <tr key={productId || product.sku} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{product.reorderLevel}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center gap-3 justify-end">
                      {canCreate && (
                        productId ? (
                          <Link href={`/products/${productId}/edit`} className="inline-flex items-center gap-1 text-primary hover:text-primary/80">
                            <Edit className="w-4 h-4" /> Edit
                          </Link>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-muted-foreground/60 cursor-not-allowed">
                            <Edit className="w-4 h-4" /> Edit
                          </span>
                        )
                      )}
