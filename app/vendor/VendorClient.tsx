  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const mRes = await fetch(`/api/admin/vendor-metrics?vendorId=${vendorId}`);
        if (mRes.ok) {
        ...
        // Fetch My Shipments
        const sRes = await fetch(`/api/shipments?vendorId=${vendorId}`);
        if (sRes.ok) {
          ...
          <div className="lg:col-span-1 space-y-8">
            <div className="bg-white/5 border border-white/10 rounded-3xl p-8 relative overflow-hidden group shadow-2xl">
              ...
              <div className="flex items-baseline gap-2">
                 <h2 className="text-7xl font-black text-white">{metrics?.reliabilityScore || 94}</h2>
              ...
              <div className="mt-8 space-y-4">
                 <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-emerald-400 font-black">{100 - (metrics?.delayRate || 5)}%</span>
                 </div>
                 <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${100 - (metrics?.delayRate || 5)}%` }} />
                 </div>
                 <div className="flex justify-between items-center text-sm">
                 ...
                 <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-6 group hover:border-blue-500/50 transition-all cursor-pointer">
                    <div className="flex items-start justify-between mb-4">
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                             <p className="text-xs font-black text-white">{sh.id}</p>
                             <p className="text-[10px] font-bold text-slate-500 uppercase">{sh.type}</p>
                          </div>
                       </div>
                       <div className="text-right">
                          <p className="text-[10px] font-black text-slate-500 uppercase">Current ETA</p>
                          <p className="text-xs font-black text-blue-400">
                             {sh.eta ? new Date(sh.eta).toLocaleDateString() : 'N/A'}
                          </p>
                       </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-sm mb-6">
                       <div className="flex-1">
                          ...
                          <p className="font-bold text-slate-300 truncate">{sh.destination?.name || sh.destination?.warehouseId}</p>
                       </div>
                       <ArrowRight className="w-4 h-4 text-slate-600" />
                       <div className="flex-1 text-right">
                          <p className={`text-[10px] font-black uppercase mb-1 ${sh.riskScore >= 65 ? 'text-red-500' : 'text-emerald-500'}`}>
                             {sh.riskScore >= 65 ? 'High Risk' : 'Optimal'}
                          </p>
                          <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                             <div 
                                className={`h-full rounded-full ${sh.riskScore >= 65 ? 'bg-red-500' : 'bg-emerald-500'}`} 
                                style={{ width: `${sh.riskScore}%` }} 
                             />
                          </div>
                       </div>
                    </div>
