...
export async function GET(
  ...
  try {
    ...
    const delivery = await getDocument<any>('deliveries', params.id);
...
export async function PUT(
  ...
  try {
    ...
    await updateDocument('deliveries', params.id, updateData);

    const updated = await getDocument<any>('deliveries', params.id);
  ...
  try {
    ...
    if (action === 'validate') {
      ...
      await adminDb.runTransaction(async (t) => {
        const docRef = adminDb.collection('deliveries').doc(params.id);
