...
export async function POST(request: NextRequest) {
  try {
    ...
    const countSnapshot = await adminDb.collection('deliveries').where('warehouseId', '==', warehouseId).count().get();
    const deliveryNumber = generateReferenceNumber('WH', warehouse.code, 'OUT', countSnapshot.data().count + 1);
