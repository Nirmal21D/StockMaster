...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate receipt number by checking count
    const countSnapshot = await adminDb.collection('receipts').where('warehouseId', '==', warehouseId).count().get();
    const receiptNumber = generateReferenceNumber('WH', warehouseDoc.code, 'IN', countSnapshot.data().count + 1);
