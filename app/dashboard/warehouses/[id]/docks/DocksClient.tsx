...
function DocksClientContent() {
  ...
  const handleReschedule = (slotTime: string) => {
    setRescheduling(slotTime);
    setTimeout(() => {
      setRescheduling(null);
      // In a real app index, we would send a PATCH to update the shipment ETA
      fetchSchedule(); 
    }, 2000);
  };
                ...
                <div className="md:text-right">
                   {slot.atCapacity ? (
                     <button 
                        ...
                        disabled={rescheduling === slot.time}
                     ...
                     >
                        {rescheduling === slot.time ? 'REBALANCING CONFLICT...' : 'RESCHEDULE PRIORITY LOAD'}
                        {rescheduling !== slot.time && <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />}
                     </button>
                   ) : (
